...
const fetchKMBRoutes = async () => {
  try {
    ...
    if (!response.ok) {
      console.warn('KMB routes fetch failed:', response.status);
      return [];
    }
...
const fetchKMBRouteStops = async (route, direction, serviceType = 1) => {
  try {
    ...
    if (!res.ok) {
      console.warn('KMB route-stop fetch failed:', res.status);
      return [];
    }
    ...
    const enrichedStops = await Promise.all(stops.map(async (s) => {
      try {
        ...
        if (!stopRes.ok) return { ...s, name_en: 'Unknown Stop', name_tc: '未知' };
        const stopData = await stopRes.json();
        return { ...s, name_en: stopData.data?.name_en || 'Unknown Stop', name_tc: stopData.data?.name_tc || '未知' };
      } catch (e) {
...
const fetchKMBETA = async (stopId, route, serviceType = 1) => {
  try {
    ...
    if (!res.ok) {
      console.warn('KMB ETA fetch failed:', res.status);
      return [];
    }
    const data = await res.json();
...
const fetchMTRETA = async (line, station) => {
  try {
    ...
    if (!res.ok) {
      console.warn('MTR schedule fetch failed:', res.status);
      return { up: [], down: [] };
    }
    const data = await res.json();
    ...
    if (!data.data || !data.data[key]) return { up: [], down: [] };
